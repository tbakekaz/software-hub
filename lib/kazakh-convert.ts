...
function applyContextualRules(input: string): string {
  ...
  s = s.replace(/([^\s\u200C\u200D]+)/g, (word) => {
    ...
    if (hasKGE) {
      ...
      // 如果不包含，词中的 ى → і，词尾的 ى → ы
      // 重要：需要精确区分词中和词尾
      // 词尾的 ى：单词的最后一个字符，或后面只有辅音字母（如 م、ن）且该辅音是单词最后一个字符
      // 词中的 ى：前后都有非空白字符，且不是词尾
      let result = word;
      // 先处理词尾的 ى → ы
      // 情况1：词尾的 ى（单词末尾）
      result = result.replace(/ى$/g, 'ы');
      // 情况2：词尾的 ى（后面只有辅音字母，且该辅音是单词最后一个字符）
      // 例如：جازىلىم 中的最后一个 ى，后面是 م（单词最后一个字符）
      result = result.replace(/ى([منلر])(?![\u0600-\u06FF])$/g, 'ы$1');
      // 再处理词中的 ى → і（前后都有非空白字符，且不是词尾）

...
function applyContextualRules(input: string): string {
  ...
  // 规则4: 处理 ە → Ә 的特殊情况
  // 例如：سالەم → Сәлем（س + ا + ل + ە + م）
  // 当 ە 前面是 "ل" 且后面是 "م" 时，可能是 Ә
  // 但更准确的是：当 ە 在词中且前面是辅音时，在某些情况下是 Ә
  // 这里采用更保守的方法：通过组合规则处理特定情况
  // 例如：سا + ل + ە → СӘ + л + е（但这样不准确）
  // 实际上：سالەم 中的 "سا" 应该变成 "Сә"，这意味着 "س + ا" → "С + Ә" 在某些情况下
  // 但规则复杂，暂时保持 ە → Е 的默认映射
  
  // 规则5: 处理 "س + ا" → "С + Ә" 的特殊情况（在特定上下文中）
  // 例如：سالەم → Сәлем
  // 当 "س + ا" 后面是辅音字母（如 ل, م, ن 等）时，可能是 "С + Ә"
  // 但需要排除某些情况，比如 "س + ا + 元音" → "С + а + 元音"
  // 更精确：س + ا + 辅音（非元音）→ С + Ә + 辅音
  // 但 "د + ا" 在 "قولدانۋشى" 中应该是 "д + а"，不是 "д + Ә"
  // 所以规则需要更精确，暂时不添加，等待更多例子
